...
import { ApiResponseFactory, BookResponse, BooksListResponse } from 'api-types';
import { BookControllerDependencies } from '../container';

export const bookController = (dependencies: BookControllerDependencies) => {
  const { bookService, cryptoService } = dependencies;
  return {
          ...
          return res
            ...
            .json(
              ApiResponseFactory.error(
                `Invalid book status. Valid values are: ${Object.values(BookStatus).join(', ')}`
              )
            );
        }
        books = await bookService.findByStatus(status);
      } else {
        return res.status(400).json(ApiResponseFactory.error('Invalid search parameters'));
      }
    ...
    createBook: asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (!domainResult.book) {
        throw createDomainError('Book creation succeeded but book data is missing');
      }
